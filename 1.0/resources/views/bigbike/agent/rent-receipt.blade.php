...
@section('content')
    ...
    @if(Session::has('rent_success'))
        ...
        <div class="text-center" >
            ...
            <div id="main-text">
                <div class="text-center " ><p><strong>Central Park Bike Tours<br> address: {{ $location->title }},<br>
                            New York, NY {{ $location->zipcode }}
                    ...
                    <p>Rent Date: {{ str_replace("/","-",$agent_rents_order['date']) }}<br>
                        Duration:<span id="duration-js">{{ $agent_rents_order['duration'] }}</span><br>
                        Rent Time: <?php echo $tmp->format('h:ia '); ?><br></p>

                    <p> Must return by:<br><strong style="font-size:19px;"><span id="return">{{ $test[0] }}</strong></span><br></p>
                    {{ $test[2] }}<br>
                    ...
                    {{--@endif--}}
                        ...
                        @if($agent_rents_order['payment_type']=='Credit Card') <p>Paypal Transaction #: {{ $agent_rents_order['order_id'] }} <br>@endif
                            ...
                            @if( $agent_rents_order['mountain']!='0')Mountain Bike: <strong>{{ $agent_rents_order['mountain'] }}</strong><br>@endif
                                @if( $agent_rents_order['kid_trailer']!='0')Trailer: <strong>{{ $agent_rents_order['kid_trailer'] }}</strong><br>@endif
                                @if( $agent_rents_order['electric_bike']!='0')Electric Bike: <strong>{{ $agent_rents_order['electric_bike'] }}</strong><br>@endif
                                @if( $agent_rents_order['elliptigo']!='0')Elliptigo: <strong>{{ $agent_rents_order['elliptigo'] }}</strong><br>@endif

                                @if( $agent_rents_order['tricycle']!='0')Tricycle: <strong>{{ $agent_rents_order['tricycle'] }}</strong><br>@endif
                                @if( $agent_rents_order['electric_hand']!='0')Electric Hand Cycle: <strong>{{ $agent_rents_order['electric_hand'] }}</strong><br>@endif
                                @if( $agent_rents_order['snow']!='0')Snow Bike: <strong>{{ $agent_rents_order['snow'] }}</strong><br>@endif

                            @if( $agent_rents_order['trailer']!='0')Tagalong: <strong>{{ $agent_rents_order['trailer'] }}</strong><br>@endif
                        ...
                        @endif
                            ...
                            Total @if($agent_rents_order['payment_type']=='coupon')( <span id="coupon">{{$agent_rents_order['payment_type']}}</span>)@endif:${{ number_format(floatval($agent_rents_order['total_price_before_tax']),2) }}<br>
                                ...
                                <div class="text-center" >
                                    ...
                                    <div id="main-text">
                                        <div class="text-center " >
                                            ...
                                            <br><br><br>
                                            <div>Bike Rent & Bike Accessories</div>
                                            @php
                                                $total_tmp = number_format(Session::get("inv_cart")["price"]+floatval($agent_rents_order['total_price_after_tax']),2);
                                            @endphp
            ...
            <div class="receipt-text"><strong>If you have any problems with your bikes in central park, please call us using the toll free 800 number:1-800-772-7174</strong>
            </div>
...
@if(Session::has('location') && (Session::get('location')=='203W 58th Street'))
    <script>
        ...
        if (f!== null) {
                ...
                console.log('A');
//
           if ($("#duration-js:contains('All Day (8am-8pm)')").length){

            var b = document.getElementById('return');
            var c = document.getElementById('duration-js');

               b.innerHTML = '6:00pm';
                       c.innerHTML = 'All day (8am-6pm)';
           }
        }
    </script>@endif
